...
class Database {
    ...
    func schema(_ table: String) -> DataResults? {
        ...
        let fields = query(sql)
        return fields
    }
    
    func browseAll(_ table: String) -> DataResults? {
    ...
    func recordCount(_ table: String) -> Int {
        ...
        guard let rows = query(sql),
            let row = rows.first,
            let val = row["count"] else {
                return 0
        }
        return val as! Int
    }
    ...
    func execute(_ sql: String, values: [Any]? = nil, params: [String:Any]? = nil) -> Int {
        var result = -1
